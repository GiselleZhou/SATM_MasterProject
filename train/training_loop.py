...
class TrainLoop:
    def __init__(self, args, train_platform, model, diffusion, data):
        ...
        # If fine-tuning with new layers, skip loading old optimizer state
        # (parameter count mismatch: new layers have no optimizer history)
        if hasattr(args, 'finetune_from') and args.finetune_from:
            self.resume_step = 0
            print('[Fine-tune] Starting fresh optimizer (new layers detected)')
        ...
        if 'image' in self.cond_mode:
            ...
            self.scene_loader = SceneImageLoader(scene_dir, clip_preprocess)
            print('Scene image loader initialized for image-conditioned training')
    ...
    def _load_and_sync_parameters(self):
        ...
        if resume_checkpoint:
            ...
            if missing:
                print(f'[Fine-tune] New layers initialized randomly: {missing}')
            if unexpected:
                print(f'[Fine-tune] Unexpected keys in checkpoint: {unexpected}')
        ...
        for epoch in range(self.num_epochs):
            ...
            for motion, cond in tqdm(self.data):
                ...
                if self.scene_loader is not None:
                    cond['y']['image'] = self.scene_loader.sample(motion.shape[0]).to(self.device)
