...
class CompMDMGeneratedDataset(Dataset):
    ...
    def __init__(self, model, diffusion, dataloader, mm_num_samples, mm_num_repeats, max_motion_length, num_samples_limit, scale=1.):
        ...
        # If model uses image conditioning, inject random scene images during eval
        scene_loader = None
        if hasattr(model, 'cond_mode') and 'image' in model.cond_mode:
            import clip
            import os as _os
            ...
            print('Scene image loader initialized for evaluation')
        ...
        with torch.no_grad():
            for i, (motion, model_kwargs) in tqdm(enumerate(dataloader)):
                ...
                # Inject scene images if model uses image conditioning
                if scene_loader is not None:
                    model_kwargs['y']['image'] = scene_loader.sample(motion.shape[0])
